...
def collect_r2c_data(data_root, mode='train', record_file='./data/refsplits.json'):
    ...
    ref_iamges_dir = os.path.join(data_root, 'Ref', 'Images')
    assert os.path.exists(ref_iamges_dir)
    
    camo_classes = os.listdir(camo_Imgs_dir)
    ref_classes = os.listdir(ref_iamges_dir)
    ...
    for c_idx in tqdm(range(len(camo_classes))):
        ...
        ref_cate_iamges_dir = os.path.join(ref_iamges_dir, cate)

        ref_cate_split_names = splits[mode if mode != 'val' else 'test'][cate]
        ref_image_dict_list[cate] = [os.path.join(ref_cate_iamges_dir, ref_cate_split_names[f_idx]+'.jpg') for f_idx in range(len(ref_cate_split_names))]

    print('>>> {}ing with {} r2c samples'.format(mode, len(image_label_list)))
    return image_label_list, ref_image_dict_list
